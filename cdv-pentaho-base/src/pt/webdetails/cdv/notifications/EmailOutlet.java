...
public class EmailOutlet implements NotificationOutlet {
  ...
  private String getSubject( Alert alert ) {
    return "CDV: [" + alert.getLevel().toString() + "] " + alert.getSummary();
  }
  ...
  private static class EmailAuthenticator extends Authenticator {
    ...
    protected PasswordAuthentication getPasswordAuthentication() {
      final String user = PentahoSystem
          .getSystemSetting( "smtp-email/email_config.xml", "mail.userid", null ); //$NON-NLS-1$ //$NON-NLS-2$
      final String password = PentahoSystem
          .getSystemSetting( "smtp-email/email_config.xml", "mail.password", null ); //$NON-NLS-1$ //$NON-NLS-2$
