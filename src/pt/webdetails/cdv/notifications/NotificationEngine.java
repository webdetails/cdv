  ...
  private NotificationEngine() {
    try {
      ...
      if (CdvEnvironment.getPluginRepositoryReader().fileExists( CdvConstants.SolutionFiles.NOTIFCATIONS ) ) {
        doc = saxReader.read( CdvEnvironment.getPluginRepositoryReader().getFileInputStream( CdvConstants.SolutionFiles.NOTIFCATIONS ) );
      } else {
        doc = saxReader.read( CdvEnvironment.getPluginSystemReader().getFileInputStream( CdvConstants.SolutionFiles.NOTIFCATIONS ) );
      }
